...
(function ($) {
    ...
    $("#price-range").slider({
        ...
        values: [100, 1000],
        slide: function (event, ui) {
            $("#priceRange").val("가격 : [ " + ui.values[0].toString().replace(/\B(?<!\.\d*)(?=(\d{3})+(?!\d))/g, ",") + " - " + 
											  ui.values[1].toString().replace(/\B(?<!\.\d*)(?=(\d{3})+(?!\d))/g, ",") + " ]" + " 원");
        }
    });
    $("#priceRange").val("가격 : [ " + $("#price-range").slider("values", 0) + " - " + $("#price-range").slider("values", 1) + " ]" + " 원");
